...
`;
    ...
    class WebGLManager {
      ...
      updatePalette(newColors: string[]) {
        ...
        if (mat.uniforms.palette) {
          ...
          mat.uniforms.palette.value = makePaletteTexture(newColors);
        }
      }
      dispose() {
        try {
  ...
  useEffect(() => {
    ...
    // Check if colors changed, and if so, update palette dynamically
    const colorsJoined = colors ? colors.join(',') : '';
    const prevColorsJoined = prevColorsRef.current.join(',');
    if (colorsJoined !== prevColorsJoined) {
      webgl.updatePalette(colors || []);
